...
docs/       design documents (132 files; requirements → preliminary → detailed, per phase)
```
...
| **20** | **DPI awareness**: the process declares Per-Monitor V2, and the DIP contract the framework has carried since Phase 13 finally takes effect -- the public API stays in DIP, conversion happens only at the platform boundary, and the backend keeps working in physical pixels. Until now the process was DPI-unaware, so the system virtualised everything and `dpi` was always 96, which made the conversion a no-op: the contract happened to agree with reality without ever being exercised. This also closes the `WM_NCHITTEST` misalignment Phase 13 recorded and left open, and adds the screen and DPI queries that were missing. Worth noting about the order of work: declaring awareness and keeping the DIP semantics correct have to land together, because declaring alone would shrink windows and text by a third on a 150% display. | ✅ Requirements **v1.1 approved** · ✅ preliminary design **v1.1 approved** · ✅ detailed design **v1.3 approved** · ★★ **fully closed** (2026-09-24): five batches implemented, the crash at non-96 DPI fixed, and the acceptance passed -- identical at 100%, correct at 125%/150%, and correct across monitors. Second item of the defect queue. The review round asked for three things to be nailed down in the detailed design — the `PixelsToDip` rounding, the `WM_DPICHANGED` chain, and what actually happens when the awareness declaration fails. The last one became **§2.3.1**: the framework **reads** the DPI rather than assuming V2 took effect, so all three paths stay self-consistent -- a failed declaration on an unaware process reads 96 and the conversion degenerates to identity, which is the G5 guard rather than a half-downgrade. The earlier requirements round added **K13** — the client size has *three* entrances (construction-time `GetClientSize`, `OnResized`, `WindowResizedEvent`), the survey having covered only the event one, missing `OnResized`, which is what actually feeds the layout — and **§4.1**, noting that the *same* `GetClientRect` must yield DIP on the platform side and physical pixels in the backend, so the two must never be merged into one size accessor. |
...
| **21** | **System icons to Image**: the third image source. WIC already decodes `.ico` *files* through `DecodeFile`, but a runtime `HICON` -- the icon the shell hands you for a path -- had no way to become pixels (`GetIconInfo` / `GetDIBits` / `SHGetFileInfo` had zero hits). Adds that path, producing the existing `Image` (premultiplied BGRA), so `Decode` gains its third source alongside memory and file. Open question: testability, since shell APIs may not run headless. | 🚧 Requirements **v1.0 under review** (2026-09-25). Clears G-2, which the desktop roadmap lists as its only blocker. |
