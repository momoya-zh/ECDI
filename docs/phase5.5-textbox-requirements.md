...
> 状态：v1.1（2026-08-13）｜职责确认完成——✅ 已实现（2026-08-13，5.5.1 MVP：码点编辑/光标/点击定位/裁切）
...
- **修正（评审）**：推迟到 5.5.2（与修饰键一起，Shift+方向键依赖修饰键）。
...
- **IME 记账（评审 提醒）**：TextBox 支持 UTF-8（直接码点输入）但**中文输入法组合实际走 WM_IME_***——5.6 IME 完成前"能显示中文、不能通过输入法输中文"。提前记账，避免做到一半才发现。
...
- v1.0（2026-08-13）职责确认：T1-T9 定稿。T4 分阶段（评审 修正：Selection 是编辑系统横切面，推迟 5.5.2）；T5 位标志 KeyModifier（评审 修正：可组合）；实施拆 5.5.1 MVP / 5.5.2 高级（评审 重排 + 修正 Selection→修饰键依赖序）；IME 提前记账。
- v1.1（2026-09-11）实现落地状态同步（补记）：5.5.1 MVP 已于 2026-08-13 实现；头部状态由「待初步设计」回写为「已实现」。
