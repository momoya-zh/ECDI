...
> 版本 v1.2（2026-08-29，外部评审整合：初步设计通过）。五阶段法第二步：方案空间 + 倾向。**✅ 已实现（2026-08-30）**
> 优先级（requirements v1.1 §5 / 外部评审锚定）：**先把 tick 路由通道设计干净，再谈 Animator API**——路由是结构性问题，API 是表面积。
...
- TimerEvent 自带 `Window*`——结构性免查找（评审 担心的"怎么找到对应 Window 的 manager"答案就在事件自身）
- `Application::OnTimer` 是 timer 语义唯一汇合点，分支路由 = 语义最自然处
- 改动最小：1 处分支 + 1 个 Window 转发方法；TimerEvent 派发语义零改动（8.5.1 模型原样保留）
- **外部评审（2026-08-29）**：「非常干净……这就是典型的'能通过已有对象关系解决，就不要新造全局管理结构'」——方案 A 定稿
...
- 选项 a：**`std::chrono::steady_clock`，每 tick 用真实 elapsed 推进**——倾向（平台无关标准库；帧率无关、抖动免疫；不引入虚拟时钟抽象，YAGNI）——**外部评审确认定稿**：WM_TIMER 是"大约 16ms 来一次"而非"严格每 16ms"，固定步进会让动画实际变慢、真实 elapsed 保持总时长正确；"Animation 300ms 是时间意义上的 300ms，不是 19 个 tick"
...
- 详细设计必答议题（记议题不展开）：
  ...
  - **重复启动语义**：同目标同属性再启动 = 替换式重启（**新 from = 当前呈现值**，防跳变）——S1 hover 快速进出的真实场景；**外部评审明确赞成（2026-08-29）**：「from 应该是当前呈现值，而不是上一次动画的 target，否则快速 Hover Enter→Leave→Enter→Leave 很容易产生颜色跳变」——**此子议题视为已收敛**，详细设计不再重议
...
- **S2 的定位（外部评审锚定）**：S2 不只是"让数字从 100 变成 200"，而是验证**动画系统能否成为现有 GUI 状态/布局/渲染体系的一个正常消费者**——高度变化 → Geometry 改变 → Layout → 子控件位置 → Invalidate → 重 Paint 全链路
- **Demo → CollapsiblePanel 顺序（外部评审确认）**：先基础设施 → 首个消费者 → 第二消费者验证 → 再按 demo 验证过的实现模式产品化正式控件——CollapsiblePanel 不是"为验证动画临时造的控件"，而是模式验证后的正式化

## 6. 决策点汇总

| # | 议题 | 倾向 | 定稿时机 |
|---|---|---|---|
| d1 | TimerId 分配约定 | 集中 `TimerIds.h`（TextBox 别名迁移） | 本阶段确认（评审 未反对） |
| d2 | 路由方案 | A：`Application::OnTimer` 保留 id 分支 | ✅ 外部评审确认（2026-08-29） |
| d3 | 时间源 | steady_clock 真实 elapsed | ✅ 外部评审确认（2026-08-29） |
| d4 | 循环模式 | 挂账（spinner 消费者出现再评估） | 已定 |
| d5 | 目标悬挂 / 重复启动语义 | 弱关联 + 替换式重启（from = 当前呈现值） | 重启子项 ✅ 评审 赞成已收敛；悬挂子项详细设计 |
| d6 | 值应用形态 / 目标绑定 | —— | 详细设计（评审：S2 是真实验证场，重点审） |
...
- v1.1（2026-08-29）外部评审整合（✅ 初步设计通过，可进详细设计）：① d2 方案 A 确认定稿（「能通过已有对象关系解决，就不要新造全局管理结构」）；② d3 steady_clock 确认定稿（「300ms 是时间意义上的 300ms，不是 19 个 tick」）；③ d5 重复启动语义 评审 明确赞成、视为已收敛（from = 当前呈现值）；④ §5 新增 S2 定位锚定（验证动画能否成为 GUI 体系的正常消费者）+ Demo→CollapsiblePanel 顺序确认（先基础设施→首消费者→第二消费者验证→再产品化）；⑤ 决策点汇总表状态同步；⑥ 无拒绝项。评审 附加提示：详细设计重点审 Animation / AnimationManager / 目标绑定 / 重启 / 完成回调的具体结构，S2 布局联动链是真实验证场。
- v1.2（2026-09-11）实现落地状态同步（补记）：9.6 动画系统已于 2026-08-30 实现；头部状态由「可进详细设计」回写为「已实现」。
