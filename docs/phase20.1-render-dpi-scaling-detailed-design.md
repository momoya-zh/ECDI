...
> 状态：**v1.0**（2026-09-24）——**待评审**（评审通过后方可进实现）
...
| **§11** | `≤1px` 不作无条件定理 | ✅ 已在初设 v1.1 §2.4 收紧；本稿 **§8-O1** 沿用同一措辞 |
...
**★ A4 / C-11 的 grep 判据式样**（★ 必须区分**代码行与注释**）：

```text
① helper 定义各 1 处：
   grep -n '^\s*\(Rect\|Point\|float\) Scale\(Rect\|Point\|Length\)(' src/Render/Renderer.cpp   -> 应为 3

② 乘法的唯一落点：形如 `<expr> * scale` 的**代码行**只应出现在三个 helper 体内（合计 7 行：
   ScaleRect 4 + ScalePoint 2 + ScaleLength 1）⇒ grep -n '\* scale' src/Render/Renderer.cpp 计数 = 7

③ 反向排除：本判据命中注释行时须剔除（`//` 开头）；且 src/Render/ 之外**不得**出现 `ScaleRect` 等标识符
```
...
⇒ **目的**：防后人写出 `auto image = cmd.image;`（复制）或对 `image` 做任何"顺手"处理——**`image` 必须原样引用到后端**。
