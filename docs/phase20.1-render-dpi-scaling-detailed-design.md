...
> 状态：**v1.3**（2026-09-24）——✅ **三批全部落地 · 四工具链 + 测试全绿（247 → 252）· ★★ 人工目视验收通过**（用户原话「**跨屏拉取已经正常了**」⇒ **`RG-1` 修复生效**）；★ **实施记录见 §12**。v1.2 = 批一的 §5 判据口径修正（见 §11）；v1.1 = 评审通过「**可进入实现**」
...
| **批三** | `Window.cpp`（△3，注入 `GetDpiScale()`） | ★ **四工具链** + **目视**：**100% 下与改前一致** & **125% / 150% 下内容充满窗口**（**`RG-1`** 修复生效） ✅ |
...
| **2** | ★ **Phase 20 详设 §14.5 的 `RG-1` 标 ✅**，并指向本子阶段 ✅ |
...
- **v1.3**（2026-09-24）**实施记录 —— ★ 三批全部落地 + 四工具链 + 人工验收通过**。① **实施结果**：批一 / 批二 / 批三全部完成；**四工具链**（MSVC / Clang / ClangCL / MinGW）构建通过 · 测试**全绿**（用例 **247 → 252**）· ★★ **人工目视验收通过**（100% 与改前一致 · **125% / 150% 内容充满窗口** · **跨屏完全正常**）。② ★★ **新增 §12 实施记录**（实施规模 · 1 处判据偏离 · 3 条设计层新增论证 · A1–A7 验收状态）。③ ★ **§7 三批 / §9 的 A1–A7 / §10 收口清单 5 项全部标 ✅**。④ ★ **术语正名**：本缺口原在子阶段文档中称 `R1`，因与**需求稿 §6 的 `R1`（需求条目）撞车**而**统一正名为 `RG-1`**，正式登记处 = **Phase 20 详设 §14.5**（§7 / §10-2 两处引用已同步）。⑤ 头部 v1.2 → **v1.3**。
...
## 12. 实施记录（v1.3，2026-09-24）

> **三批全部落地 · 四工具链构建 + 测试全绿（247 → 252）· ★ 人工目视验收通过**。

### 12.1 实施规模

| 项 | 值 |
|---|---|
| **批数** | 3（批一 `Renderer.{h,cpp}` · 批二 `RendererTests.cpp` · 批三 `Window.cpp` 注入） |
| **改动文件** | **4 代码**（`Renderer.h` · `Renderer.cpp` · `Window.cpp` · `RendererTests.cpp`）+ 文档 3（本子阶段三件套） |
| **用例数** | **247 → 252**（★ **扩既有 `RendererTests.cpp`** ⇒ `RunAllTests.*` **零登记改动**） |
| **公共头** | **92 → 92**（★ **0 新增头**；`Renderer.h` 是**既有**公共头，只加 **1 处带默认值的形参**） |
| **新增 `#include`** | **0**（★ **连 `<cmath>` 都不需要**——helper 不取整，**B13 兑现**） |
| **零改动项** | `Renderer::BeginFrame`（**声明 / 定义 / 注释一字未动**）· 三个后端（`RenderingBackend` / `GDIBackend` / `RecordingBackend`）· `RunAllTests.*` · `CMakeLists.txt` · `main.cpp` |
| **构建验证** | ★ **四工具链**（MSVC / Clang / ClangCL / MinGW）全部通过 · 测试全绿 |

### 12.2 实施期的偏离（★ 仅 1 处，且在批一）

| # | 详设原文 | 实施 | 判据 |
|---|---|---|---|
| **①** | §5 的 A4 / C-11 grep 判据：「`* scale` 合计 **7 行**」· 两条「应 0」判据**未提示排除范围** | **判据文本就地修正**（本稿 → **v1.2**，见 §11） | ★★ **不是实现偏离，是判据偏离实测**：① 「**处**」与「**行**」被混为一谈——**7 处落在 6 行**（`ScalePoint` 那行含两次）；② 两类必须排除的行被遗漏——**注释行**（本文件注释里**故意**写着 `[[maybe_unused]]` / `(void)scale` 作为「**不做**」的说明）与 **helper 定义行**（`Rect ScaleRect(` 也含同名标识符）。⇒ ★ **只动判据文本，不改任何设计决定**（△1–△4 · C-7–C-11 · 三批顺序**全不变**） |

### 12.3 设计层的新增论证（实施 / 验证期拿到）

| # | 内容 |
|---|---|
| **1** | ★ **本地真跑配方改进**：**有现成 CMake 构建目录时，`ninja -C <build-dir> <target>` 优于裸编译器手工配方**——旗标天然等于 `CMakeLists.txt` 这个唯一真相源，免去手工库列表（正是此前漏 `gdi32` 的根源），且**增量**（改一个 cpp 只重编 1 个 TU + 重链） |
| **2** | ★ **零行为变化的硬证据（顺手可得）**：`AntiAliasingTests` 打印的覆盖度积分（`sum(c/255)=0.811765 / 50.352941 / 804.262745 / 50.254902`）**与改造前逐位相同** ⇒ 比「测试全绿」更强的「**没改行为**」证据 |
| **3** | **A2 的第二条理由（性能）**：除「多后端共用缓冲会**语义污染**」外——★ 命令持 `std::string` / `Image` 像素缓冲 ⇒ 「**复制命令再折算**」等于**每帧深拷贝文本与整张图** |

### 12.4 验收状态（§9 的 A1–A7）

| 项 | 状态 |
|---|---|
| **A1** 150% 内容充满窗口 | ✅ 用户目视（125% / 150% / **跨屏**全部正常） |
| **A2** 100% 逐位零回归 | ✅ `T20.1-1` + **本地真跑**（MinGW + `-D_DEBUG`，**252 全绿**）· AA 积分逐位相同 · 四工具链 |
| **A3** 缩放 / 移动 / 最大化同源同尺度 | ✅ 目视（命中侧 Phase 20 已折，本稿补**绘制侧**） |
| **A4** 落点唯一可 grep | ✅ **§5 三条判据**（口径已按 v1.2 修正） |
| **A5** 四工具链 + 测试全绿 | ✅ ★ **四工具链通过** · **252 全绿**（断言层启用：MinGW 侧 `-D_DEBUG` **169** 条） |
| **A6** 顺序 / 类型 / 数量不变 | ✅ `T20.1-5` ① |
| **A7** 覆盖每种几何字段 | ✅ `T20.1-3`（`Rect` / `Point` / `width` / `cornerRadius` + **`image` 内容不变**） |