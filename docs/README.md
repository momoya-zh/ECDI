...
## Phase20 DPI 感知（DPI awareness）（🚧 需求确认 **v1.0 待评审**，2026-09-24 立项）
...
| 文档 | 内容 | 状态 |
|------|------|------|
| [phase20-dpi-awareness-requirements.md](phase20-dpi-awareness-requirements.md) | 需求确认（**K1–K12 现状勘察全部带行号**——★ K1 DIP 契约已立 · K3 `DipToPixels` 的 DPI 来源与**预留替换点** · ★★ **K5 进程未声明感知 = 全部症状的根因** · ★★ **K6 = Phase 13 的 L6/D6 明写「一并闭合」**；**§1.3「两条错误互相抵消」**；**G1–G5** 目标（含逐位等价护栏）· **N1–N5** 非目标 · **路线 A/B/C**（倾向 **B**）· ★ **§3.2 连锁关系** · **§4 触面清单**（10 个面 + 「后端不需变」）· **D0–D8** 决策点 · **R1–R9** 需求 · **A1–A6** 验收（区分自动 / 人工）· **Q1–Q6** 留给初设（★ Q2「文本测量单位」是最实际的取舍）） | 🚧 **v1.0 待评审** |
