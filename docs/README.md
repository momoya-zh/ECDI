...
- **Phase 18 窗口/根背景能力 —— 需求确认 v1.0 🚧 待评审**：`roadmap-deferred.md` **§7.7 #39** 的立项落地（2026-09-21，**由 Phase 17 A6 实施实测派生**）。**动机**：Phase 17 的 **R3「root 一级即全局留白」在视觉上不可用**——root 无背景 ⇒ 让出的四边露出 Backend 清屏白（`#0f1115` 深色窗口上一圈白框），且标题栏被连带内缩。**缺口实测（K1–K4）**：客户区底色是 `GDIBackend.cpp:261` 的**硬编码 `WHITE_BRUSH`**，**无任何可配置入口**（无 `Window` API、无 `RenderingBackend` 接口位、root 亦是裸 `Widget`）。**路线倾向**：**A 窗口级配置 + Backend 消费**（B 让根具备背景为备选；C 不做为兜底）。详见下方 Phase18 段。
...
## Phase18 窗口/根背景能力（Window/Root background）（✅ 需求确认 **v1.1 评审通过**（2026-09-22）· ✅ **初步设计 v1.1 评审通过**（冻结 **O1–O4** · T18-5 语义收窄 · 用例口径 **226 + 5 = 231**）· 🚧 **详细设计 v1.0 待评审**）
