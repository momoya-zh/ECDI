...
> **当前规模锚点（防止各处历史数字误读）**：测试 **210** 用例（`GetTestRegistry().Add` 求和，**21 个含用例的测试文件**——`src/Tests/*.cpp` 共 24 个，其中 `RunAllTests.cpp` / `TestFramework.cpp` / `test_main.cpp` 为基础设施无用例）｜Public 头 **92**（`include/ECDI/**/*.h`，另 `Core/version.h` 为 CMake 生成头不计）｜设计文档 **119** 篇（`docs/**/*.md` 递归，含 `docs/model-probe/` 2 篇；顶层 117 篇）。下表各阶段状态栏内的数字为**该阶段实现时点值**，非当前值。
...
## Phase16 桌面驻留层（`WindowLayer::Desktop`）（✅ 需求确认 v1.4 · 🚧 初步设计 v1.2——评审通过 · **O1 已关闭**，可进详细设计）
...
| [phase16-desktop-layer-preliminary-design.md](phase16-desktop-layer-preliminary-design.md) | 初步设计（**§1 设计输入与基线**：D0–D11 已定项汇总 · **B1–B9 代码基线（带行号）** · **§1.3 ★ 需求稿勘误**（K13 实核不成立）· §1.4 §8.2 八问答案索引；**§2 头文件改动**——**公共头净增 0**（**不新增 pure virtual ⇒ 3 个替身零同步**）+ `Win32PlatformWindow.h` 内部头增量；**§3 实现分解**——`TargetInsertAfter` 分流（含 `nullptr` 哨兵语义）· `ApplyDesktopStyle` 幂等可逆 · **`hook → 实例` 反查** · 重插前判在位 · `Release` 顶部脱钩（**修既有 `hwnd` 空判陷阱**）· 四态语义「零代码」对照；**§4 契约 C1–C12** · **§5 影响面** · **§6 开放决策点 O1–O5 + §6.1 O1 实测方案** · **§7 测试 T16-1..7**） **v1.1（2026-09-19）外部评审处置**（「基本通过，带 3 项收敛进详细设计」）：**★ O3 定为 C**——拆出**纯函数** `ResolveTarget(layer, desktop)`（public static，**零新文件**）⇒ 最危险的 C2 分支（桌面不可用 ⇒ **绝不**降级 `HWND_BOTTOM`）由「需 O3 / 代码审查」升级为 **T16-7 全自动**；**★ O2 倾向反转**为「**不加** `WS_EX_TOOLWINDOW`」（它改的是任务栏 / Alt+Tab / 激活 / 系统菜单语义，**不属本阶段需求**；「桌面常驻物不该占任务栏」成立但属**未立项需求**）；**新增契约 C11 / C12**（不可判 ⇒ 禁止无依据 z-order 操作 · **钩子独立于 `HWND`**，`Release()` 判空不得覆盖脱钩）；**修 §5 方法计数**（+10 → **+12**）· **修 §7 自洽**（原「唯一不可自动 = C1」与 §3.7 矛盾）· **补 B10**（测试已 include 内部头）；**★ O1 由「倾向不需要」升级为「详设前置实验」**——本稿此前推理有缺口：既有 4 组是**创建期**设样式，框架是**创建后** `SetWindowLongPtrW`，**两者不等价** ⇒ 新增 §6.1（探针 v4 `--strip-after` / `--framechanged`，打印样式回读 + 非客户区尺寸前后对比）。 **v1.2（2026-09-19）O1 实测关闭**：三组 × 3 轮 Win+D **全 PASS**（`--strip-after` / `+ --framechanged` / 创建期烘焙），**加与不加 `SWP_FRAMECHANGED` 行为完全一致**，且组 1 的「改样式前后非客户区尺寸**逐位不变**」（`16×39 → 16×39`）为决定性读数 ⇒ **`ApplyDesktopStyle` 不需要 `SWP_FRAMECHANGED`**；**框架先例**：`SetChromeMode(Borderless)` 已在配置期派发过一次（`Win32PlatformWindow.cpp:800-804`）。**如实记录一处探针保真度缺陷**——v1–v5 漏了框架 `SetChromeMode` 的 `SWP_FRAMECHANGED`，故其 `non-client=16×39` 读数在框架里不会出现（**对 P0 / O1 结论均无影响**，已修于探针 **v5a**）；并**修正 v1.1 的判据表述**（delta ≠ 0 是**我们派发**的效果，不是系统自发）。新增 **B11** · §5「工具」行 · §6 标题与 §3.2 定案。 | 🚧 **v1.2——O1 已关闭（不需 `SWP_FRAMECHANGED`），可进详细设计** |
