...
> **当前规模锚点（防止各处历史数字误读）**：测试 **218** 用例（`GetTestRegistry().Add` 求和，**22 个含用例的测试文件**——`src/Tests/*.cpp` 共 25 个，其中 `RunAllTests.cpp` / `TestFramework.cpp` / `test_main.cpp` 为基础设施无用例）｜Public 头 **92**（`include/ECDI/**/*.h`，另 `Core/version.h` 为 CMake 生成头不计）｜设计文档 **125** 篇（`docs/**/*.md` 递归，含 `docs/model-probe/` 2 篇；顶层 123 篇）。下表各阶段状态栏内的数字为**该阶段实现时点值**，非当前值。
...
## Phase16 桌面驻留层（`WindowLayer::Desktop`）（✅ 需求确认 v1.4 · ✅ 初步设计 v1.3 · ✅ 详细设计 v1.10 · 🚧 **已实施（批 A/B/C + A5 跟随修复 + T16-8 + 中断安全修正）——待验收 A1–A8**）
...
## Phase17 布局内边距（Layout padding）（✅ 需求确认 v1.0 · ✅ 初步设计 v1.1 · 🚧 详细设计 v1.0 待评审）
