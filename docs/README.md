...
- **Phase 15 滚动容器（ScrollView + 滚动条）（✅ **已实现并验收** 2026-09-18；2026-09-17 立项）**——**R1 内核 + R2 滚动条同 Phase 交付**（用户拍板）；首个真实消费者 = `examples/ModelProbe/` 手搓 35 行的 `ModelListPanel`（**已收回——净删 35 行**）；**兑现 Phase 9.5 R1 的「不做水平滚动条」记账**（该记账仅指 TextBox——本阶段是容器级）。核心结构判断：滚动容器的真问题是**坐标语义（偏移要进命中）+ 可见性约束（滚出视口不可命中）**，而非「多画一块」（裁切能力 9.5 R1 已备）。**v1.1（2026-09-18）**：外部评审「方向通过，可进初设」——**D3 经全库枚举升级为「必须认偏移」（5 消费者全是视觉坐标，3 处为换算链）**；**D2 核实 `CaptionBar` 不受影响（opt-in）且语义定案为「点检查」**；**D7 采纳评审建议降级为记账**（嵌套消费者实测 0）；新增设计不变量（坐标三分 + 职责四层）与 K15–K18 勘察事实。详见 [phase15-scrollview-requirements.md](phase15-scrollview-requirements.md)（**✅ v1.2 已通过** 2026-09-18——二轮外部评审「可进初设」；**D1–D12 全部拍板**，§1.4 四条不变量为强约束；**初设 ✅ v1.2 已通过**（三轮评审「可进详设」）· **详设 ✅ v1.2 已实现**（A/B/C 三批 + 验收实测）——见 [phase15-scrollview-detailed-design.md](phase15-scrollview-detailed-design.md) 与 [phase15-scrollview-preliminary-design.md](phase15-scrollview-preliminary-design.md)）。**验收实测（2026-09-18）**：四工具链构建通过 · `ecdi_tests` **210 全绿**（196→210，与详设预测一致）· **A2 断言启用核验 10/10 ✅**（★ MinGW **首次**由「死代码」转为启用——`CMakeLists.txt` 的 `_DEBUG` 补丁生效）· A4 偏移层隔离通过 · ModelProbe 手测「滚轮一滚一行 28px / 条可拖拽与轨道翻页」。**实现期三处纠偏**：① **偏移消费者下沉**——`ScrollView` **不** override 偏移，新增内部节点 `ScrollContent` 作内容坐标空间之根（否则滚动条作为直接子会被一起偏移 ⇒ 视觉/命中/绝对坐标三处同错）② **事件坐标系**——`ScrollBar` 初版把事件坐标当局部用（**真缺陷**，用户实测发现）⇒ `MainAxisPosFromClient` 完成「客户区绝对 → 局部」换算 ③ **extent 给出方式**——改用显式 `SetContentExtent`（行池隐藏行会污染「子控件包围盒」推导）。 **A1–A7 验收全过**（补记 2026-09-18：A6 = `ecdi_public_header_test` **92 头独立编译 ✅**，且作编码探针揪出 `Font.h` 缺 BOM 的 `C4819`；A7 = CLion Reload CMake Project ✅）。
