...
> **当前规模锚点（防止各处历史数字误读）**：测试 **231** 用例（`GetTestRegistry().Add` 求和，**23 个含用例的测试文件**——`src/Tests/*.cpp` 共 26 个，其中 `RunAllTests.cpp` / `TestFramework.cpp` / `test_main.cpp` 为基础设施无用例）｜Public 头 **92**（`include/ECDI/**/*.h`，另 `Core/version.h` 为 CMake 生成头不计）｜设计文档 **132** 篇（`docs/**/*.md` 递归，含 `docs/model-probe/` 2 篇；顶层 **130** 篇）。下表各阶段状态栏内的数字为**该阶段实现时点值**，非当前值。
...
## Phase19 鼠标事件维度补全（Mouse event dimensions）（需求 **v1.1 ✅ 评审通过** · 初步设计 **v1.0 🚧 待评审**——2026-09-22 立项 / **2026-09-23 进初设**）
