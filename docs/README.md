...
> **当前规模锚点（防止各处历史数字误读）**：测试 **218** 用例（`GetTestRegistry().Add` 求和，**22 个含用例的测试文件**——`src/Tests/*.cpp` 共 25 个，其中 `RunAllTests.cpp` / `TestFramework.cpp` / `test_main.cpp` 为基础设施无用例）｜Public 头 **92**（`include/ECDI/**/*.h`，另 `Core/version.h` 为 CMake 生成头不计）｜设计文档 **123** 篇（`docs/**/*.md` 递归，含 `docs/model-probe/` 2 篇；顶层 121 篇）。下表各阶段状态栏内的数字为**该阶段实现时点值**，非当前值。
...
- **Phase 17 布局内边距（Layout padding）—— 需求确认 v1.0 🚧 待评审**：`roadmap-deferred.md` §7.6 **#38** 的立项落地（2026-09-20 用户拍板**方案 B（框架侧）**）。**动机**：内容四边贴死客户区、鼠标拖选极易越出 GUI 边界。⚠️ **归因澄清**：贴边**不是 AutoSize 的缺陷**（AutoSize 的定义即「窗口尺寸跟随内容」⇒ 内容从 `(0,0)` 铺满是其必然结果），**修复落点在布局层**。**路线**：`VerticalLayout` / `HorizontalLayout` 各加 `int padding = 0` 构造形参、`Arrange` 改 **4 处** ⇒ **root 一级配置即全局留白**；现有调用点（生产示例 20 / 测试 20 / README 示例 1）**全部零改动**。详见下方 Phase17 段。
...
## Phase17 布局内边距（Layout padding）（✅ 需求确认 v1.0 · 🚧 初步设计 v1.0 待评审）
