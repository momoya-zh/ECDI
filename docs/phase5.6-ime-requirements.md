...
- **否决 B（组合串内嵌）**：评审 论证——B 是"文本系统 2.0"非体验增强，引入 6 个问题（组合串是否 Text 一部分 / Selection / Backspace / Delete / 取消组合 / 候选切换），全是编辑系统升级，超 Phase 5 范围
- **否决 C（裁剪）**：用户倾向做，A 解决最明显的体验问题
- **5.6 一句话定义（评审）**：**不改 TextBox 编辑系统，只补 Window↔IME 桥接层**
...
- 否决 `GetCaretScreenPosition()`（评审：ScreenPosition 是平台概念，TextBox 只负责客户区，Window 负责 ClientToScreen——职责清晰）
...
- **做**：WM_IME_STARTCOMPOSITION 处理（候选跟随）+ WM_IME_ENDCOMPOSITION **空通道预留**（评审建议——START/COMPOSITION/END 生命周期完整，未来 B 升级通道就位）
