...
> 状态：v1.1（2026-08-26）｜v1.1 = 外部评审整合（9.5/9.6 边界明确化 + R4 前置定位 + R1 职责修正）
...
> v1.1 明确（外部评审）：Phase 9.6 动效系统已立项，**9.5 与 9.6 边界必须分开**——不把 9.6 内容塞进 9.5，也不因 9.6 把 9.5 硬凑成"动画前置阶段"。
...
- **定位**：R1 是 **Clip 栈机制的消费层**（Phase 8 提供能力 → 9.5 建立使用规则），**不是完整的 Dirty Region 系统**（评审 v1.1 修正）
- **范围**：绘制管线 Clip 规则 + TextBox 裁切改造
- **明确不做（YAGNI）**：Dirty Region 区域追踪（Invalidate 保持全窗口 dirty）、脏区像素级追踪/多区域合并/复杂 InvalidateRect 语义

### 决策点
- **d1 绘制阶段 Clip 规则形态**：Paint 管线统一 PushClip(自身边界)（嵌套交集）vs 控件自行负责——倾向 **统一管线规则**（防越界绘制），但**具体形态留待详细设计再审**（嵌套交集语义 + 影响面广——评审 提示详细设计阶段认真审）
- **d2 TextBox 裁切**：PushClip 替代字符串截断——倾向 **随 9.5 一起做**（已记账债务 + Phase 8 PushClip 已就绪）
- **d3 Invalidate 语义**：保持全窗口布尔，不引入区域追踪——**确定**（YAGNI，性能瓶颈未实证）

## 3. R2 LinearLayout 抽象 ✅ 评估关闭（2026-08-28：确认不抽）
...
### 评估结论（2026-08-28，代码事实核实）
**确认不抽，R2 关闭记账**。理由 4 条：
1. **抽象后代码不减反增**——`LinearLayout(Axis)` 基类（~30 行）+ 2 个壳子类 + Axis 枚举 ≈ 现状 89 行（两实现各 27 行、核心逻辑各 7 行），还多间接层
2. **重复成本可忽略**——核心逻辑各 7 行；"diff 同构约束"（仅 y→x / height→width 镜像）本身就是防漂移机制，改一处必改另一处、评审可见
3. **第三用例未出现**——Wrap/Grid/Flex 未立项，抽象方向（Axis 参数化未必是 Wrap 的形状）没有依据，过早抽象可能抽错方向
4. **YAGNI**——第二次用例出现前不抽（6.1 立项原判定，维持）
重启条件：第三布局用例（Wrap/Grid/Flex 之一）立项时。

## 4. R3 WM_MOVE 场景 ✅ 评估关闭（2026-08-28：确认不做）
...
### 评估结论（2026-08-28，代码事实核实）
**确认不做，R3 关闭记账**。现状核实：`WM_EXITSIZEMOVE` → `m_host.OnExitSizeMove()` → 框架层销毁+重建 caret 强制 TSF 缓存失效（候选窗归位）——移动/缩放结束的归位机制完整（5.6 实测 + 8.5.1 落地）；`WM_MOVE`（移动中）未处理。理由 4 条：
1. **模型 B 已消除主要视觉错位**（8.5.1）——组合串由 TextBox 自绘、随窗口移动；移动中错位的仅剩**系统候选窗**（候选词列表 UI）
2. **场景极罕见**——候选窗显示（打字中）与窗口拖动（鼠标占用）互斥；键盘移动窗口（Alt+Space+M）更是边缘操作
3. **现有方案足够**——EXITSIZEMOVE 归位：拖动结束瞬间立即恢复正确位置，错位仅存在于拖动过程中的短暂视觉
4. **成本收益**——WM_MOVE 实时跟随实现成本低，但收益 ≈ 0；且 PlatformWindowHost 加 OnMove 回调 = 框架契约变化（d1 修正项）
重启条件：真实用户遇到并困扰（如触摸/笔输入边拖边打字场景）时再评估。
...
## 6. R5 Shortcut System / 键盘入口统一 / InputManager ✅ 评估关闭（2026-08-28：三子项全部关闭）
...
### 评估结论（2026-08-28，代码事实核实）
**三子项全部关闭**。现状核实：`OnKeyDown` → `Window::HandleKeyDown`（Tab 导航预处理后派焦点控件）；`OnKeyUp`/`OnCharInput` → `FindFocusedWidget` 直派焦点控件——不对称确认，但**是设计决策非缺陷**（Tab 导航只需 KeyDown 时处理，KeyUp/CharInput 无导航需求）。结论：
1. **键盘入口不统一**——统一=增加无消费场景的间接层；TextBox 的 Ctrl+C/V/X/Z/Y 由控件 OnKeyDown 自处理，工作正常
2. **全局快捷键不做**——无框架级消费场景（无 Application 级 Ctrl+S 之类需求）；消费场景出现时再评估（届时 d1/d2 决策点可直接用）
3. **InputManager 不引入**——输入分发（Application→Window→Widget）清晰，抽象层收益未证
重启条件：框架级快捷键消费场景出现（如 Application 注册 Ctrl+S 保存）；OnKeyUp/CharInput 需要 Window 预处理的需求出现。
...
- v1.1（2026-08-26）外部评审整合：① §0 新增 9.5/9.6 阶段边界（9.5 = 收尾基础设施，9.6 = 动效系统，不互塞）；② R4 定位升级 = **9.6 前置交互基础设施**，职责写死"只产生 Hover 事实、不负责视觉过渡/动画/Hover Style"，依赖提升为正式（9.6 首个消费者）；③ R1 职责修正 = **Clip 栈消费层**（绘制阶段 Clip 生命周期规则 + TextBox 裁切迁移），明确非 Dirty Region 系统；d1 形态留待详细设计再审；④ 无拒绝项。
- v1.2（2026-08-28）**R2/R3/R5 评估关闭**：R2 LinearLayout 确认不抽（抽象后代码不减反增、第三用例未出现）；R3 WM_MOVE 确认不做（模型 B 组合串自绘随窗消除主要错位、打字与拖窗互斥）；R5 三子项全部关闭（键盘入口不对称=设计决策、无全局快捷键消费场景、InputManager 收益未证）。9.5 五项全部收口：R4/R1 已实现，R2/R3/R5 关闭记账。
