...
| **8（复用）** | **DPI 感知**（系统级 → Per-Monitor V2；并补**屏幕 / DPI 查询 API**） | **真缺陷**（无替代路径） | ② | **不新开号**——本条即 §1.1 条 **8**「DPI 感知评估」的**优先级提升**；证据补 Phase 16 spike 实测跨屏 reparent 后矩形 ×0.8（`desktopnest-roadmap.md:247`）。⚠️ **触面广**（坐标换算点散布）⇒ 先定 API 形态再动 · **✅ 已立项——Phase 20**（2026-09-24）· 需求稿 `docs/phase20-dpi-awareness-requirements.md`（**v1.0 待评审**）· ★ **本项的特殊性**：**不是引入新概念，而是让既有契约生效**——DIP 契约早在 **Phase 13** 就已声明（`Win32PlatformWindow.cpp:376`），并留下 **L6**（`WM_NCHITTEST` 物理像素 vs widget DIP 几何）明写「与 DPI 感知一并闭合」（`phase13-captionbar-detailed-design.md:795/1051`） |
