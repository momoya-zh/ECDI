...
| **8（复用）** | **DPI 感知**（系统级 → Per-Monitor V2；并补**屏幕 / DPI 查询 API**） | **真缺陷**（无替代路径） | ② | **不新开号**——本条即 §1.1 条 **8**「DPI 感知评估」的**优先级提升**；证据补 Phase 16 spike 实测跨屏 reparent 后矩形 ×0.8（`desktopnest-roadmap.md:247`）。⚠️ **触面广**（坐标换算点散布）⇒ 先定 API 形态再动 |
