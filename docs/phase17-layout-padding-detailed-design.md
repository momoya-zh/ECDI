...
| **C6** | **负值钳 0 + Debug 断言** | △1（`padding < 0 ? 0 : padding`）＋ △2（`FRAMEWORK_ASSERT(padding >= 0)`） | **T17-8**（Release 钳 0）+ **A2**（断言特征串 11/11） |
...
| # | 判据 | 手段 / 口径 |
|---|---|---|
| **A1** | 四工具链 **Debug** 构建通过；`ecdi_tests` **226 passed, 0 failed** | 用户（VS 2026 / CLion）。复跑：`cmake --build <dir> --target ecdi_tests` → 运行 |
