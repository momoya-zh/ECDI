...
- 不抽 `LinearLayout(Direction)`：`if/else + 状态` 反而复杂（评审 论证）
- 触发条件：第三个方向（Wrap/Grid/Flex）出现 → 再评估 LinearLayout 基类

### H7 CheckBox / Radio — 决策点 d：**本轮不做** ✅（评审 强烈建议）
...
评审 提议 Phase 6 拆 6.1-6.4（HorizontalLayout / Clipboard / Copy-Cut-Paste / Shortcut）。**本轮只做 6.1 布局**——Clipboard/Copy-Paste/Shortcut 是**原路线图外的新需求**，各属独立功能域，**不并入本轮**（避免范围膨胀）。记入路线图未来候选，各自独立走五阶段法。
