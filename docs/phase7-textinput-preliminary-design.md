...
## P1 CaretGeometry.h（新文件，Widget/ 领域目录——评审 赞成）
...
## P2 TextBox 升级（h/cpp）——评审 修订：改名 + 常量提取

```cpp
// TextBox.h：
CaretGeometry GetCaretClientGeometry();   // ⚠️ 评审：GetCaretClientPosition → GetCaretClientGeometry
                                          // （返回值已是 CaretGeometry，Position 名不副实）

// TextBox.cpp：
namespace{   // 匿名 namespace：光标视觉常量（评审：不散落魔法数字）
...
void Win32PlatformWindow::UpdateTextInputCaret(const CaretGeometry& geometry){
	// 7.1.3：visible 判断在**平台表现层**（评审：Window 不知道 CreateCaret/HideCaret 细节）
	...
	// 光标竖线由控件 OnPaint 自画）。visible=true 不做 ShowCaret（与 评审建议的分歧点：
	// 评审 认为 ShowCaret 配 HideCaret，但 ECDI 自画光标——ShowCaret 会产生双光标）。
	HideCaret(m_hwnd);
