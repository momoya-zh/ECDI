...
> 状态：**v1.2**（2026-09-23）——✅ **评审通过**（外部评审第二轮 2026-09-23，**无阻塞项**）
...
| **用例数** | **231 → 240**（+9）。★ **v1.2 勘误**：按**注册条目**口径实为 **231 → 236**（**5 条注册**承载 T19-1..T19-9）——实现与收口均以 **236** 为准（详见详设 §5.2） |
...
| **A3** | 零回归：既有用例全绿 | `ecdi_tests` **236 全绿**（231 既有 + 5）；**须写明断言是否启用** |
...
| **③** | **`FakeHost` 测试装置的扩展** | **§7**（`ReceivedEvent` +2 字段 · 两个既有分支各补 2 行 · T19-1..T19-9）；详设须给出**改动后的 `OnEvent` 片段**＋ ★ **v1.2 更正**：**不涉及** `RunAllTests.h` / `RunAllTests.cpp`（本项**扩展既有** `EventTests.cpp`、**无新文件**——详设 B15） |
...
- **v1.2**（2026-09-23）**实现后回填 + 两处口径更正**。① ★ **用例数口径更正**：本节 §5 与 §9 原写 **231 → 240**（把 **T 编号数**当用例数）——按**注册条目**口径实为 **231 → 236**（5 条注册承载 T19-1..T19-9）；两处数字与 §9-③ 的「登记两处」一并更正（**登记不涉及** `RunAllTests.*`）。② **实现与实测完成**：四工具链 **236 全绿** · 断言特征串 **11 → 11** · 公共头 **92 → 92** · **公共 API +2** · **8 文件改动 / 无新文件**。③ ★ **P1/P2 的实测方案已执行**（见详设 §6.1）：另**附带测得 P3**（`WM_MOUSEMOVE` 按住时置位）——**这正是本项核心用途可成立的证据**。④ **内容零删改**（本稿所有要求 / 决策 / 非目标 / 验收判据原样保留，只改两处口径）。⑤ 头部 v1.1 → **v1.2**。
