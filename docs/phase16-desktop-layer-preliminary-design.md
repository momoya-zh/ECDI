...
### 1.2 代码基线勘察（B1–B9，2026-09-19 复核）
...
⚠️ **是否需 `SWP_FRAMECHANGED`**：`WS_MINIMIZEBOX` **不参与非客户区几何计算**（只控制系统菜单项与按钮可用性）⇒ 预计**不需要**。但此为**待实测项**——列为 **O1**。若实测发现系统标题栏的最小化按钮仍可点，才补 `SetWindowPos(…, SWP_FRAMECHANGED)`。
