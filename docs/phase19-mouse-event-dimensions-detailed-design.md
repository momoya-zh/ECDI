...
| # | 预期 | **实测** | 结论 |
|---|---|---|---|
| **P1** | `WM_LBUTTONDOWN` 置 `MK_LBUTTON` | `LOWORD(wParam)` = `0x0001`，`MK_LBUTTON` = **1** | ✅ **成立**——与既有测试约定（`EventTests.cpp:148`）一致 |
| **P2** | `WM_LBUTTONUP` 不含 `MK_LBUTTON` | `LOWORD(wParam)` = `0x0000`，`MK_LBUTTON` = **0** | ✅ **成立**（抬起时该位**已清**） |
| ★ **P3**（**附带测得**，本稿原未列） | — | `WM_MOUSEMOVE` 在左键按住时 `MK_LBUTTON` = **1** | ✅ **拖动期间可读**——**本项核心用途成立** |

**方式**：**独立探针**（`.workbuddy/tmp/p1p2_probe.cpp`，**零仓库侵入**、不进仓库）——用户在窗口内点击左键 + **按住拖动**。**探针用后即删**。

**★ 三条里最重要的是 P3**：P1 / P2 只覆盖**两个端点**；而 Phase 19 的**核心用途是「按住拖动」**——若 `WM_MOUSEMOVE` 的 `wParam` 不带 `MK_LBUTTON`，`MouseMoveEvent` 的 `IsButtonDown()` 就**永远为 false**，本项等于落空。⇒ 三条合起来证明 `IsButtonDown()` 在「**按下 / 拖动 / 抬起**」三个阶段**全部可读**。

**给消费者的直接结论（本表的主要产出）**：

| 时机 | `IsButtonDown(Left)` | 含义 |
|---|---|---|
| `WM_LBUTTONDOWN` | **true** | 「**按下即开始拖**」**可以**直接依赖它（不必再自己维护布尔） |
| `WM_MOUSEMOVE`（左键按住） | **true** | 拖动判断可直接用 |
| `WM_LBUTTONUP` | **false**（该位已清） | ★ **抬起那一刻不能**再用它判断「刚才是否在拖」——须用 `GetButton()` 或消费者自持状态 |

★ **与 D4 的关系（精确）**：P3 说明「**拖动中判断按键**」这一**局部**确实可由 `IsButtonDown()` 承担，但 `ScrollBar::m_dragging` / `TextBox::m_mouseDown` 还承担**捕获**与**拖动 / 拖选生命周期** ⇒ **D4 结论不变**（本项只补信息，不重构它们）。

**未改动**：代码 / 用例 / 契约——三条实测**全部与预期一致** ⇒ 按上表「纪律」行，**连注释都无需调整**。

**★ 装置设计的一处失手（教训）**：首版探针把结果只画在**客户区**（`TextOutA`）⇒ **GUI 客户区不能选中 / 复制**，用户**只能手打回传**。⇒ **正确做法：结果追加写一个文本文件**（用 `GetModuleFileNameA` 推导 exe 同目录路径，避免落到仓库根），客户区只作实时指示。**判据：「让用户读屏」的装置很难回传结果；能复制或落文件才行。**
