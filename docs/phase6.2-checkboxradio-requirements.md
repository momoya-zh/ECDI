...
| # | v1.0 决策 | 对齐更新 | 依据 |
|---|---|---|---|
| **D1** | 样式"颜色沿用 Button 模式，Phase 9 主题替换"（v1.0 §4） | **直接带 CheckBoxStyle/RadioStyle 进 Theme**——Theme 新增 `GetCheckBoxStyle()/GetRadioStyle()`，DefaultTheme 提供默认值；StateWidget 持有 Style（Phase 9 StyleField 机制：ApplyTheme/SetStyle） | Phase 9 已落地（评审 v1.4 明确："CheckBox/Radio 落地时直接带 CheckBoxStyle/RadioStyle"） |
...
### C4 状态变化通知 — 决策点 d：**OnCheckedChanged(bool checked)** ✅（评审 修订）
```cpp
...
// 不暴露 Toggle()——Radio 不能取反（评审：Toggle 接口对 Radio 语义错误）
```
