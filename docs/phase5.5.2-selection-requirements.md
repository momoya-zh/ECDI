...
- KeyEvent 加 `m_modifier` + **`HasModifier(KeyModifier)` 辅助**（评审 补充：位与判断）+ `IsShiftDown()/IsCtrlDown()/IsAltDown()`（基于 HasModifier）
...
### S1 Selection 数据模型 —— A ✅（评审 修订：anchor/active 语义）

- **`m_selectionAnchor`（锚点，固定端）+ `m_caret`（活动端 active）**——不是 start/end！
- 关键洞察（评审）：**光标 = active 端**——Shift+←→ 就是移动光标；`ab|cdef → Shift+→ → ab[c]def → Shift+← → ab|cdef`（收缩回 anchor，标准行为）
...
- **`m_mouseDown`（TextBox 内部状态）**——Down 置 true / Up 置 false；与 Window Capture 机制解耦（Capture 是窗口级机制、拖选是控件行为——评审：不绑死，未来程序释放 Capture/失焦状态可推导）
...
### S3 Shift+方向键 —— A ✅（评审 修订：anchor 固定 + active 移动）

- Shift+←/→：移动 m_caret（active），anchor 不动
- Shift+Home/End：扩展到头/尾
- 无 Shift 的 ←/→/Home/End：**清选择**（anchor 无效化）+ 移动光标

### S4 编辑感知选中区（Selection 横切面）—— A ✅（评审：DeleteSelection 返回新 caret）

- **`size_t DeleteSelection()`**（评审：返回删除后光标位置 = min 处）——删除逻辑抽一次，三处复用
...
- **高亮与文本裁切共享 maxTextWidth**（评审：文本裁切 → Selection 裁切 → 光标钉右缘 共享同一可视宽度——高亮不溢出）
- 高亮块 = GetSelectionMin/Max 前缀宽度区间（两次 MeasureText）

### S6 Ctrl+A —— ⚠️ 推迟（评审：用户预期一致性）✅
