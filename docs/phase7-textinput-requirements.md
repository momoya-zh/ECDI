...
### D1 CaretGeometry 落点 — Widget/ 领域目录（评审 修订）
`ECDI/Widget/CaretGeometry.h`——只有文本输入领域消费（TextBox→Window→PlatformWindow→IME），不是 Point/Rect/Color 全框架基础类型。**迁 Core 条件（评审 标准）：被至少 3~4 个独立子系统使用**（TextArea/CodeEditor/RichText 出现后）。

### D2 CaretGeometry 可扩展结构（评审 修订——非 Rect 空包装）
```cpp
