...
| 版本 | v1.2 |
| 日期 | 2026-08-30 |
| 状态 | **已实施（2026-08-30，外部评审全项授权 + 确认）**——编译验证已通过（README 索引 ✅） |
...
|---|---|
| v1.0 | 初版 |
| v1.1 | 外部评审吸收：§3.3 增收起态 SetSize 轴语义**硬契约**（非动画轴保持 / 动画轴为 0）；初始化约定明确为「文档约定、不做运行时 assert」；§6.2 测试补四向轴断言 |
| 实施 | 2026-08-30 授权，按 §8 顺序落地全部 9 项文件；待用户编译验证（AI 仅静态自检） |
| v1.2 | 2026-09-11 **实现落地状态同步**（补记）：本文件 9 项文件清单已于 2026-08-30 按 §8 顺序落地，用户编译验证已通过；状态行去掉「待用户编译验证」，顺延项以 README 技术债表为准 |
...
**硬契约（收起态 SetSize 轴语义，外部评审 v1.1 补充）**：收起态 `SetSize` 只定义展开基准，呈现几何按轴拆分——**非动画轴保持 SetSize 给出值，动画轴呈现 0**：
...
- 上述两条均为**初始化约定**（文档 + 头文件注释表达），**不引入运行时 assert / 强制校验机制**——「用户按约定初始化即可」（外部评审 v1.1 确认，YAGNI）。
