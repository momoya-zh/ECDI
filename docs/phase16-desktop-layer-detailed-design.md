...
> 阶段：详细设计（五阶段法 ③）
> 日期：2026-09-19
> 状态：**✅ 已实现并关闭**（2026-09-21）——评审通过（2026-09-19「设计上通过，可以进入实现」）→ 三批实施 + A5 跟随修复 + T16-8 → **`ecdi_tests` 218 全绿**（2026-09-20 实测，移除全部诊断后取得）→ **A1–A8 验收**：判据① 实测通过、②–⑥ 由用户 2026-09-21 确认关闭；A7 收尾本次完成（`desktopnest-roadmap.md` §5 **G-1 标 ✅**）。评审给出的 **7 条实现级检查项**已成 **§9.1 盯防清单**。初设 v1.3「评审通过 · O1 已实测关闭」（`SWP_FRAMECHANGED` **不需要**）。
...
| # | 项 | 判据 |
|---|---|---|
| **A1** | 四工具链构建 + 测试 | MSVC / ClangCL / Clang / MinGW 全绿；`ecdi_tests` 210 → **218**。★ 其中 **MinGW / Clang 侧须确认本阶段唯一新增的平台回调**（`CALLBACK` 签名 + `HWINEVENTHOOK` 类型 + 链接）**实际过一遍**——同型先例（`&Win32PlatformWindow::WindowProc`）**不可外推**，`WINEVENTPROC` 是另一个函数指针类型（§9.1 #7） — **✅ 2026-09-21** 四工具链均构建过（四 debug 目录 obj/exe 时间戳实证，`ecdi_tests.exe` 均 09-21 12:27 重链）；`ecdi_tests` **218 全绿**（clang 实测） |
| **A2** | **断言启用核验** | 按 skill 条 50 取二进制证据：**10 条条件串**（★ **本阶段零新增断言** ⇒ 特征集**不变**）——`grep -c` 判据同 Phase 15（clang `-D_DEBUG` / clangcl `-MDd` / mingw `-D_DEBUG` / visual-studio `-MDd`） — **✅** 本阶段**零新增断言** ⇒ 特征集不变（10 条）；Phase 17 实测 **11 条全在**（含覆盖本阶段的 10 条） |
| **A3** | **零回归** | 既有 210 用例**全部保持通过**——尤其 `WindowChromeTests` 9 条（`RuntimeApiRejectedBeforeShow` / `ChromeModeDecidedOnce` / `MaximizedClientWithinWorkArea`）与 `AnimationTests` / `ProgressBarTests` 的 `TestPlatformWindow`（3 个实现者，本阶段**无新增 pure virtual**） — **✅** 既有 210 用例全部保持通过（并入 218 全绿实测） |
| **A4** | ★ **C2 不容降级**（**本阶段特有的结构性验收**） | ① T16-7 五条全过；② **`HWND_BOTTOM` 出现点审查**：`grep -n 'HWND_BOTTOM' ECDI/src/Platform/Win32/Win32PlatformWindow.cpp` ⇒ 只允许出现在「`ResolveTarget` 的 Bottom 分支」与注释中，**不得**出现在 `WM_WINDOWPOSCHANGING` 或任何 Desktop 分支 — **✅ 已复核**：`HWND_BOTTOM` 代码行仅剩 `ResolveTarget` 分支 1 处，其余 4 处为注释 |
| **A5** | **手测**（需求稿 §6 L3 三层分层的手测层） | 六判据 + Win+D + explorer 重启 + 桌面图标可点：① Win+D 中仍可见（**允许一次瞬时遮挡**——见 L1 的判据措辞）② 被普通应用覆盖 ③ 桌面图标可点击 ④ 交互不降级 ⑤ explorer 重启后自愈 ⑥ 切回 `Normal` 后行为复原。载体：`ModelProbe --layer desktop` + `.workbuddy/spike/desktop_layer_probe.cpp` 的 `--auto` — **⚠️ 部分实测**：判据①（Win+D 后稳定留在原位）**实测通过**（2026-09-19/20）；**②–⑥ 无逐条手测记录**，由**用户 2026-09-21 决定关闭本阶段**——如实标注，不记为「已逐条实测」 |
| **A6** | `ecdi_public_header_test` | **92 头**各自独立可编译（**无新增头** ⇒ 条数不变；该目标同时是编码/BOM 探针） — **⚪ 未单独复核**：本阶段**零新增头**（92 不变）⇒ 判据与既有 `ecdi_public_header_test` 等价（Phase 17 实测 92 头通过） — **⚪ 未单独复核**：本阶段**零新增头**（92 不变）⇒ 判据与既有 `ecdi_public_header_test` 等价（Phase 17 实测 92 头通过） |
| **A7** | **文档与索引同步** | 两份 README 的规模锚点（用例 210→218 · 头 92 不变）+ `desktopnest-roadmap.md` §5 **G-1 标 ✅**（验收后）+ 需求稿 K13 勘误（需求稿 **v1.4** 已回写，验收时复核） — **✅ 本次执行（2026-09-21）**：`desktopnest-roadmap.md` §5 **G-1 标 ✅** + 头部状态（欠 4 → 3 项）+ §10 v1.8；两份 README + `MEMORY.md` 同步 |
| **A8** | **交付提示** | `CMakeLists.txt` 零改动（`GLOB_RECURSE … CONFIGURE_DEPENDS` 自动入库新 `.cpp`）；CLion 需 **Reload CMake Project**；VS 工程走 `cmake --preset vs2026`（skill 条 66） — **✅** `CMakeLists.txt` 零改动（`GLOB_RECURSE … CONFIGURE_DEPENDS` 自动入库）；CLion Reload / VS `cmake --preset vs2026` 为操作步骤 |
...
| 处 | 现状（过期） | 应改为 |
|---|---|---|
| `examples/ModelProbe/main.cpp:247` | 行尾注释：`// spike 未通过 → 降级 Bottom + Warning`（**该 spike 已于 2026-09-15 结项**，本阶段即落地 ⇒ 该注释已过期）。✅ **已实施（2026-09-19，用户授权）——实际共**两处**：`:247` 行尾注释之外，按条 29③d 全库搜旧值又发现 `:212`（`--help` 用法说明）同一过期表述——同一事实的两处复述，一并更正；diff 仅 2 行、纯注释、零逻辑改动。** | 行尾注释改为：`// Desktop 档已落地（Phase 16）：紧贴桌面窗口正上方，Win+D 后仍可见`（**仅注释、同一行内、零逻辑改动**——`layer` 的赋值语句本身不动） |

> 需求稿 **R4** 已登记此项（`main.cpp` 的过期注释）。**AI 修改 `main.cpp` 前必须取得单独授权**（skill 条 2）——本稿只登记；**已于 2026-09-19 经用户授权实施（见上）**。
...
| # | 局限 | 处置 |
|---|---|---|
| **L1** | ⚠️ **v1.7 更正：原模型把次序写反了**——原文记「外壳先抬起桌面窗口，随后前台事件才唤醒我们重插」；A5 实测为「**前台事件先到**（此刻本窗口仍在 `Progman` 正上方）→ **外壳随后才抬起桌面**」⇒ 任何「事件到达即判/即插」都会**早于**外壳动作而落空（这正是 Desktop 档在真实应用上完全失效的根因）。**已修复（A5，`5da1dca`）**：改为「延后一拍 + 固定 N 拍（16ms × 4）无条件重插」，最后一拍必然落在外壳动作之后；实测**稳定留在原位**，闪烁为**概率性**（优于原记账的「每次必有一次」）。应用侧仍不可消除闪烁本身（`WS_EX_TOPMOST` 违反「被应用覆盖」契约） | **已修复** + 判据措辞纪律：手测判据须写「**位置不变 + 随后正常显示（允许一次瞬时遮挡）**」，**不得**写「无闪烁」——那是不可满足的判据 |
...
| **L4** | 钩子**过滤**接受 `Progman` / `WorkerW`，而**目标**恒为 `GetWindow(GetShellWindow(), GW_HWNDPREV)`（「当时紧贴桌面的那个窗口」）——若某 Windows 版本「显示桌面」抬升的是别的窗口，锚点也随之改变 | 记账 + **v1.7 实测补充**：Win11 的「显示桌面」会插入 **`WinUIDesktopWin32WindowClass`** 占据「桌面正前方」——A5 日志实测它正是「外壳抬完桌面」后的重插目标（`reinsert -> target cls=WinUIDesktopWin32WindowClass`）⇒ **锚点按构造天然跟随它**，无需把它加进**过滤**集合（过滤只需认「谁被抬为前台」，实测恒为 `Progman`）。**不对称是刻意的**：过滤宁可宽、锚点必须唯一 |
...
- v1.11（2026-09-21）**Phase 16 关闭 —— A1–A8 验收回填**。① 状态行改「**✅ 已实现并关闭**」；② **§6 八条验收项逐条挂结论**（A1 四工具链构建实证 + 218 全绿 · A2 零新增断言 ⇒ 特征集不变 · A3 零回归 · A4 结构性判据复核 · **A5 判据① 实测通过、②–⑥ 由用户确认关闭（如实标注，不记为逐条实测）** · A6 未单独复核（零新增头 ⇒ 判据等价）· A7 本次执行 · A8 零改动）；③ 波及 `desktopnest-roadmap.md` **v1.7 → v1.8**：§5 **G-1 标 ✅**（含实现落点）、头部状态「两个 Phase / 欠 4 项」→「三个 Phase / 欠 3 项」、分类纪律「阻断项只余 G-2」、§10 新增 v1.8 条目；④ 两份 README 与 `MEMORY.md` 同步。**本阶段公共 API 净增 0**（首个纯实现层 Phase），改动全部收在 `Win32PlatformWindow`。
