# Phase 16 桌面驻留层（`WindowLayer::Desktop`）需求确认（v1.4）
...
> v1.4（2026-09-19）**需求稿勘误回写**（来源：初步设计 §1.3 的代码实核，非评审意见）：**K13 的断言不成立**——原写「`Bottom` 档已有跨工具链验证的 z 序用例（Phase 12）」，实核 `WindowChromeTests.cpp` 的 9 个用例**无一条**涉及 `WindowLayer`/z 序（`SetWindowLayer` 出现 0 次），Phase 12 详设 T0–T7a 亦然。⇒ **R10 是「有实现、零自动测试覆盖」**，因此 **R2「`Bottom` 语义零回归」在本阶段没有现成保护网**——初设据此新增 **T16-1** 专门补 `Bottom` 回归。⚠️ 这是「**文档声称的测试覆盖 ≠ 实际覆盖**」的实例，与 skill 条 44（grep 判据须区分代码 / 注释）同族。
...
| **K13** | ~~`Bottom` 档已有跨工具链验证的 z 序用例（Phase 12）~~ **⚠️ v1.4 勘误：实核不成立**——`WindowChromeTests.cpp` 的 9 个用例**零** `WindowLayer` 覆盖（`SetWindowLayer` 在测试代码中出现 **0 次**；`WindowLayer.h` 仅被 `#include` 而未使用），Phase 12 详设 §5.3 的 T0–T7a 同样无 z 序 / 层级用例 ⇒ **Phase 12 的 R10 是「有实现、零自动测试覆盖」** | 初设 `phase16-desktop-layer-preliminary-design.md` **§1.3**（2026-09-19 实核） | 本阶段**不得回归** `Bottom` 语义（零回归底线），且**必须由本阶段补上测试保护**（初设 **T16-1**） |
...
> ⚠️ **A5 更正（2026-09-19，详设 v1.7）**：本条作为 `WM_WINDOWPOSCHANGING` 路径的防抖手段**成立且保留**；但**不能**把它当作「桌面跟随已完成」的判据——实测「已在位」（`GetWindow(Progman, GW_HWNDPREV) == m_hwnd`）在**外壳抬桌面之前**也为真 ⇒ 用它决定「不再重插」会让跟随**完全落空**。桌面跟随已改为「延后一拍 + 固定 N 拍无条件重插」（见详设 §8 L1 的 v1.7 更正）。
...
- v1.4（2026-09-19）**K13 勘误回写**（来源：初步设计 §1.3 实核）：§1.2 的 K13 原称「Phase 12 已有 `Bottom` 档的 z 序用例」，**实核不成立**——`WindowChromeTests.cpp` 9 用例零 `WindowLayer` 覆盖、Phase 12 详设 T0–T7a 亦无 z 序用例 ⇒ 已划删除线并改注实况。**对设计的影响**：R2「`Bottom` 语义零回归」缺测试保护 ⇒ 初设新增 **T16-1** 补齐。**对流程的意义**：需求阶段的「已有能力盘点」若只靠印象而不 grep 实证，会把「有实现」误记成「有测试」。
  - 同批：初设 `phase16-desktop-layer-preliminary-design.md` **v1.0** 落地（公共头净增 0 · 基线 B1–B9 · `TargetInsertAfter` 分流 · `hook → 实例` 反查 · 契约 C1–C10 · 开放点 O1–O5 · 测试 T16-1..6）。
